...
**⚠ Status 2026-05-28: RESOLVED root cause + TRANSIENT diagnostic config (IP/MAC/hostname below are NOT stable).** The 2026-05-27 outage was the **bredOS install on the eMMC failing in early boot** (corrupt — consistent with a write interrupted during the earlier power-supply incident), NOT a hardware or power fault. Proven by a controlled SD swap-test (see [TROUBLESHOOTING.md §11](TROUBLESHOOTING.md#11-corrupt-os-install-vs-board-failure-on-an-sbc-sd-swap-test)): flashed verified **Armbian Trixie minimal** (Debian 13, kernel 6.18) to SD, **physically removed the eMMC**, booted SD-only on the same board/power/cable — nova came up on the LAN in ~3 min and serves SSH (banner `OpenSSH 10.0p2 Debian 7+deb13u2` = Debian 13, matches the flashed image via `nmap -sV`). Only OS storage changed between "dead" and "alive", so the board is **verified good**. The earlier "loading ramdisk → display cuts → no network" was the broken eMMC system dying early; HDMI blanking at kernel handoff is normal RK3588 (console on serial UART), so the real error was invisible without serial.

  - **As-observed 2026-05-28, EXPECTED TO CHANGE** (currently Armbian-on-SD, eMMC removed): IPv4 `192.168.1.228` (new DHCP lease in the .220s SBC pool); MAC `be:9e:7a:4c:d1:52` (locally-administered — Armbian generates a stable random MAC from the SoC since RK3588 has no burned-in MAC; this is *why* it differs from the bredOS-era ethernet MAC `c6:65:5c:3a:45:3e`, also board-generated, not vendor). Port 22 open; no mDNS hostname advertised yet (Armbian minimal may not have avahi up / still first-booting — no `zbred.local`/`nova.local`/`armbian.local` seen).
  - **All of IP / MAC / hostname above are transient** and WILL change again once the eMMC question is settled (reinsert+recover, or reflash eMMC to Armbian). The stable facts in the table below (.224/.221, bredOS MACs, SSH alias intent) are the **pre-flux baseline** — keep them, don't overwrite. The `nova`/`zbred` SSH aliases still pin the old `.224` and are **stale for this boot**; use raw `.228` until the eMMC fate is decided, then re-pin.
