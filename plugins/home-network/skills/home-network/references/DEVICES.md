...
| Role | Samba/SMB server (advertises `model=MacSamba` device-info); headless remote-desktop host (wayvnc); RustDesk relay (`hbbs`/`hbbr`); glances telemetry endpoint |
| SSH | enabled, port 22 |
| SSH user | `komi` |
...
| Open ports | Full live sweep from KOMI 2026-08-22 — **all of these are reachable LAN-wide**: 22 (`sshd`), 139 + 445 (`smbd`, bound to `192.168.1.165`), 5900 (`wayvnc`), 5901 (`Xtigervnc`), 8080 + 8081 (two `main` processes), 10200 (Piper TTS via `docker-proxy`), 21115 + 21116 + 21118 (RustDesk `hbbs`), 21117 + 21119 (RustDesk `hbbr`), 61208 (glances). Loopback-only, **not** LAN-reachable: 631 (CUPS), 61209 (glances server mode). |
| Services | **Piper TTS** `:10200` (Wyoming, Docker) — verified live OPEN from KOMI 2026-05-31; serves the Home Assistant voice stack (STT + wakeword run on the Nova). **RustDesk relay** — the previously-unverified note is now **CONFIRMED live 2026-08-22**: `hbbs` (pid 2517) owns 21115/21116/21118, `hbbr` (pid 2519) owns 21117/21119. **glances** `:61208` — `glances -w --bind 0.0.0.0 --port 61208 --disable-webui` (RPC only, web UI off); a second instance `glances -s -B 127.0.0.1` holds 61209 loopback-only. **`:8080` + `:8081`** — two separate processes both named `main`; owner not yet traced beyond the process name (worth identifying, since both are LAN-open with no firewall in front of them). |
| Remote desktop | **Two independent VNC servers on two ports, serving DIFFERENT desktops.** `5900` = **wayvnc** (`wayvnc.service`, runs as user `vnc`, launcher `/usr/sbin/wayvnc-run.sh`, `XDG_RUNTIME_DIR=/tmp/wayvnc`) serving the **labwc / Wayland** desktop — **HEALTHY, use this one** (verified end-to-end from KOMI 2026-08-22: authenticated and pulled a live 1280x720 framebuffer). `5901` = **Xtigervnc** (`tigervncserver@:1.service`; `/etc/tigervnc/vncserver.users` = `:1=komi`), its own X11 `LXDE-pi` session — **PERMANENTLY BLACK, do not use**; root cause + repair in [TROUBLESHOOTING.md §14](TROUBLESHOOTING.md#14-vnc-connects-and-authenticates-but-the-screen-is-black). `tigervncserver@:2.service` has been `failed` since 2026-05-24. |
| VNC auth (5900) | wayvnc offers RFB 003.008 security types 19 (VeNCrypt), 129 (RA2ne256), 5 (RA2); the only VeNCrypt subtype offered is **262 = X509Plain**; TLS 1.3 / `TLS_AES_256_GCM_SHA384` with a self-signed cert. The live `/etc/wayvnc/config` (`root:vnc 0640`) **differs from the shipped `config.pi-default`** — it drops `enable_pam=true` and instead sets a standalone `username=` + plaintext `password=`. So **5900 does NOT accept the Linux login password.** Credential lives in Bitwarden — never record it in this repo. |
| Display | **Fully headless** — both `card1-HDMI-A-1` and `card1-HDMI-A-2` read `disconnected`. labwc synthesizes a virtual output `HEADLESS-1` because `~/.config/labwc/environment` sets `WLR_BACKENDS=drm,libinput,headless` and `WLR_HEADLESS_OUTPUT=1`. Currently **1280x720@60** — that, not the client, is why the remote desktop is 720p. `~/.config/labwc/rc.xml` binds `A-F1`…`A-F8` to `wlr-randr --mode` switches (2560x1440 / 1920x1080 / 1600x900 / 1440x900 / 1366x768 / 1280x720 / 1024x768 / 800x600), so the resolution is changed from inside the session, not by editing a config. `~/.config/labwc/autostart` is intentionally empty; `~/REMOTE-DESKTOP.md` on the device documents the full picture. |
| Firewall | **NONE — `ufw` is not installed** (`dpkg -l ufw` reports `un`, i.e. never installed). Every listener above is exposed LAN-wide by default. This is the opposite posture from KOMI — see [FIREWALL.md](FIREWALL.md#hosts-on-this-lan-with-no-firewall-at-all). |
| sudo | Passwordless for user `komi` (`sudo -n true` succeeds) |
...
| Freshness 2026-08-22 | Live re-verified from KOMI and over SSH: ARP MAC matches inventory; all 12 LAN-facing ports listed above OPEN via `/dev/tcp`; `wayvnc.service` and `tigervncserver@:1.service` `active`, `tigervncserver@:2.service` `failed`; `ufw` absent; both HDMI connectors `disconnected`; `HEADLESS-1` at 1280x720; passwordless `sudo` OK. Lease `.165` still holding. |
