...
The orchestrator owns the shared hub and acceptance. Each worker owns its report. Record exact Herdr worker/workspace/pane handles, requested and observed model/effort, native session if available, and the timestamp of the latest observation. A note saying “working” is a timestamped observation, not proof of current process activity.
