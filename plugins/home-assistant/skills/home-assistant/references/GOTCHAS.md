...
- **Fix:** AL owns **color temp only** (`adapt_brightness` switch OFF, `take_over_control: false` so brightness steps don't pause color). Brightness + the night red are owned by **`pyscript/bedroom_ramps.py`** (since 2026-06-08 it is the single source of truth for AL state, gated on the master `input_boolean.bedroom_auto_lighting`): a **static 06:45→08:00 linear 0→100% morning ramp** (weekdays only, 60 s steps, override-aware — bails if measured brightness diverges >5% from last-sent) and the evening red wind-down at sunset. `adapt_color` is resumed daily at **06:45** (`cron(45 6 * * *)`) and paused at sunset, both from the pyscript. The old YAML automations `bedroom_morning_ramp` / `bedroom_al_brightness_off` / `bedroom_al_color_resume` were **removed** — don't look for them. If AL starts fighting the ramps, check `adapt_brightness` and `take_over_control` first.
