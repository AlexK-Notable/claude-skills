...
- **Large catalogs degrade selection, model-dependently.** LongFuncEval (arXiv:2505.10570, Apr 2025) — first benchmark to isolate catalog size (49→741 tools, 8K→120K tokens of definitions) — measured **7.6%–85.6%** accuracy degradation, excluding Mistral-large's 94% collapse. GPT-4o, the most robust model, dropped 10.6–13.8% (three BFCL subsets). Practitioner consensus: degradation onset **~50+ tools**, severe at 100+ — but frontier models tolerate big catalogs far better, so thresholds tuned on small models are obsolete.
